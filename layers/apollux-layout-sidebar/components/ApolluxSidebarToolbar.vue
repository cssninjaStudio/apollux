...
const showNavBurger = computed(() => {
  return (
    props.sidebar &&
    app.apollux.sidebar?.toolbar?.showNavBurger &&
    hasSubsidebar.value
  )
})
</script>
...
<template>
  ...
  >
    <ApolluxSidebarBurger v-if="showNavBurger" class="-ms-3" />

    <BaseHeading
      v-if="app.apollux.sidebar?.toolbar?.showTitle"
      as="h1"
      size="2xl"
      weight="light"
    ...
    <ApolluxSidebarTools class="h-16" />
  </div>
</template>
