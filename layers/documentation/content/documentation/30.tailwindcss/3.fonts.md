...
dark: false
---
In addition to the default `sans` and `mono` fonts, Apollux defines `heading` and `alt` fonts to allow you to create more unique designs. By default, we use predefined system fonts from Tailwind CSS but you can easily change them by editing the `tailwind.config.ts` file.
...
Apollux preinstalls [unplugin-fonts](https://github.com/cssninjaStudio/unplugin-fonts) module so you only have to configure it in your `nuxt.config.ts` file:
