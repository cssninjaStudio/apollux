...
const showNavBurger = computed(() => {
  return props.collapse && app.apollux.collapse?.toolbar?.showNavBurger
})
</script>
...
<template>
  ...
  >
    <ApolluxCollapseBurger v-if="showNavBurger" class="-ms-3" />

    <BaseHeading
      v-if="app.apollux.collapse?.toolbar?.showTitle"
      as="h1"
      size="2xl"
      weight="light"
    ...
    <div class="ms-auto flex items-center gap-x-4">
      <template v-for="tool of app.apollux.collapse?.toolbar?.tools">
        <component
