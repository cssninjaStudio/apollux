...
export interface ApolluxCollapseResolvedConfig {
  name: string
  divider?: boolean
  icon: {
    name: string
...
export function useCollapse() {
  ...
  const menuItems = computed(() => {
    if (
      ...
      app.apollux.collapse?.navigation?.items?.length === 0
    ) {
      return []
    }
    return app.apollux.collapse?.navigation?.items?.map(
      (navigation) =>
        <ApolluxCollapseResolvedConfig>{
          ...navigation,
  ...
  const header = computed(() => {
    return app.apollux.collapse?.navigation?.header
  })

  const footer = computed(() => {
    return app.apollux.collapse?.navigation?.footer
  })
