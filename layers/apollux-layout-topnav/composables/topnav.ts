...
export interface ApolluxTopnavResolvedConfig {
  name: string
  divider?: boolean
  icon: {
    name: string
...
export function useTopnav() {
  ...
  const menuItems = computed(() => {
    if (
      app.apollux.topnav?.navigation?.enabled === false ||
      app.apollux.topnav?.navigation?.items?.length === 0
    ) {
      return []
    }
    return app.apollux.topnav?.navigation?.items?.map(
      (navigation) =>
        <ApolluxTopnavResolvedConfig>{
          ...navigation,
        },
    )
  })
